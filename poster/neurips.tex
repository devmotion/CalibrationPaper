...
\begin{tcbposter}
  ...
  \posterbox[adjusted title={Motivation - what is a calibrated model?}, colback=blondsvag]{name=calibration,column=2,span=2,below=title}{
    ...
    \tcbsubtitle{ex) collision detection system}
    \begin{minipage}[t]{0.55\linewidth}
      \begin{center}
        \begin{tikzpicture}
          \node[draw, inner sep=2mm] (image) at (0, 0)
          {\includesvg[height=9mm]{car}};
          \node[above=2mm of image, anchor=base] {Input $X$};

          \node[draw, fill=gronskasvag, right=0.75cm of image, inner sep=2mm] (model)
          {\includesvg[height=9mm]{gear}};
          \node[above=2mm of model, anchor=base] {Model $g$};
          \draw [->] (image) -- (model);

          \node[draw, right=0.75cm of model, minimum height=1.3cm, font=\scriptsize, align=center] (prediction)
          {\begin{tabular}{@{}ccc@{}}
             \includesvg[width=7mm]{barrier} & \includesvg[width=7mm]{pedestrian} & \includesvg[width=7mm]{bear} \\
             80\% & 0\% & 20\% \\
           \end{tabular}};
         \node[above=2mm of prediction, anchor=base] {Prediction $g(X)$};
   ...
   If model $g$ is calibrated we know that for all inputs with this
   prediction there is an object ahead 80\% of the time, a human 0\%
   of the time, and an animal 20\% of the time.

   \begin{center}
     \begin{tikzpicture}
       \node[minimum height=1.3cm] (image) at (0, 0)
       {\begin{tabular}{@{}ccc@{}}
          \includesvg[height=3mm]{car} & \includesvg[height=3mm]{car} & \includesvg[height=3mm]{car} \\
          \includesvg[height=3mm]{car} & \includesvg[height=3mm]{car} & $\cdots$ \\
        \end{tabular}};
      ...
      {\begin{tabular}{@{}cccccc@{}}
         \multicolumn{4}{c}{\includesvg[width=5mm]{barrier}} & \includesvg[width=5mm]{pedestrian} & \includesvg[width=5mm]{bear} \\ \midrule
         \includesvg[height=3mm]{car} & \includesvg[height=3mm]{car} & \includesvg[height=3mm]{car} & \includesvg[height=3mm]{car} & & \includesvg[height=3mm]{car} \\
         $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & & $\vdots$ \\ \bottomrule
       \end{tabular}};
     \node[below=1mm of empirical] (A) {Empirical frequency $r(g(X))$};
     \node at (prediction |- A) {Prediction $g(X)$};
 ...
 \end{center}
  ...
  \posterbox[adjusted title={Calibration error}]{name=error,column=2,span=2,below=calibration}{
