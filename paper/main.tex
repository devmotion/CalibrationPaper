...
\begin{figure}[!htbp]
  \begin{center}
    \tikzsetnextfilename{#1_alpha_i=1.0_only_firstclass=false}
...
\begin{figure}[!htbp]
  \begin{center}
    \tikzsetnextfilename{#1_alpha_i=0.1_only_firstclass=false}
...
\begin{figure}[!htbp]
  \begin{center}
    \tikzsetnextfilename{#1_alpha_i=1.0_only_firstclass=true}
...
\begin{figure}[!htbp]
  \begin{center}
    \tikzsetnextfilename{#1_alpha_i=0.1_only_firstclass=true}
...
\begin{lemma}[{{Asymptotic bound based on $\linearestimator$ (see~\cref{cor:linear_asymptotic})}}]
  Let $k$ be a matrix-valued kernel as in \cref{def:kernel}, and assume that
  $\Expect[\|k(g(X),g(X))\|] < \infty$. If
  $\Var[h_{i,j}] < \infty$, then
  \begin{equation*}
    \Prob\left[\frac{\linearestimator}{\hat{\sigma} / \sqrt{\lfloor n / 2\rfloor}} \geq t \,\middle| \, \nullhypothesis \right] \to 1 - \Phi(t) \quad \text{as} \quad n \to \infty,
  \end{equation*}
...
\clearpage
\subsubsection{Distribution of estimates: Additional figures}\label{sec:additional_estimates}
...
Inception neural networks~\cite{phan19_pytor_cifar} for the classification of the
CIFAR-10 image data
set~\cite{krizhevsky09_learn_multip_layer_featur_from_tiny_images}. The CIFAR-10
